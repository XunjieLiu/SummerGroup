...
# 下面这个是用打开图片的，讲图片转化成pil 的image 对象，filepath是用来记录文件夹路径的
...
#testImg=returnNormaliezedArray(returnArray(filePath))
# 下面得到的就是图片的拉扁后的np array
# arr 是训练集中是的
arr=flatArr(returnNormaliezedArray(returnArray(filePath)))
yue=flatArr(returnNormaliezedArray(returnArray(filePath)))
# arr2是训练集中不是的
...
batch_xs, batch_ys =arr,tem1


# print(sess.run(yprev,feed_dict={x: arr2, y_: tem3}))
# 下面两句话都是在用数据集跑
sess.run(train_step, feed_dict={x: arr, y_: tem1})
...
print(sess.run(accuracy,feed_dict={x: yue, y_: tem3}))
print ("test accuracy %g"%accuracy.eval(session=sess, feed_dict={
    x: yue, y_: batch_ys, keep_prob: 1.0}))
# print(sess.run(yprev,feed_dict={x:arr2, y_: tem3}))
